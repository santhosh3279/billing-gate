...
/**
 * Get all active rate-limited and locked-out IPs
 * @returns {Array<{ ip: string, count: number, locked: boolean, retryAfterSeconds: number }>}
 */
export function getRateLimitStatus() {
  ...
  for (const [ip, record] of loginAttempts.entries()) {
    ...
    const retryAfterSeconds = isLocked ? Math.ceil((record.lockedUntil - now) / 1000) : 0;
    list.push({
      ip,
      count: record.count,
      locked: isLocked,
